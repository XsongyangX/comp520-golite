...
//forward check for string concatenation
...
//forward check for an element expression: this is to support runtime errors when trying to access out of bounds
void elementCheck(EXP *e, symTable *table, int depth)
{
	...
	switch(e->kind)
	{
		...
		case elementExp:
			...
			elementCheck(e->val.binary.rhs, table, depth);
			elementCheck(e->val.binary.lhs, table, depth);
			if(e->val.binary.lhs->kind == funcBlockExp)
			{
				tMods = getFullStr(e->val.binary.lhs, table);
				tName = getNewBinding();
				printf("struct %s %s = %s;\n", findExistingBinding(e->val.binary.lhs->symTypeRef, table), tName, tMods);
				e->val.binary.lhs->symTypeRef = makeSymbol_Copy(e->val.binary.lhs->symTypeRef);
				e->val.binary.lhs->symTypeRef->bindingName = tName;
				tName = getNewBinding();
				printf("int %s = %s; if(%s.size <= %s){fprintf(stderr, \"Access out of range.\\n\"); exit(1);}\n", tName, getFullStr(e->val.binary.rhs, table), e->val.binary.lhs->symTypeRef->bindingName, tName);
				e->symTypeRef = makeSymbol_Copy(e->symTypeRef);
				e->symTypeRef->bindingName = malloc(128);
				strcpy(e->symTypeRef->bindingName, e->val.binary.lhs->symTypeRef->bindingName);
				strcat(e->symTypeRef->bindingName, ".val[");
				strcat(e->symTypeRef->bindingName, tName);
				strcat(e->symTypeRef->bindingName, "]");
			}
			else{
				tName = getNewBinding();
				printf("int %s = %s; if(%s.size <= %s){fprintf(stderr, \"Access out of range.\\n\"); exit(1);}\n", tName, getFullStr(e->val.binary.rhs, table), getFullStr(e->val.binary.lhs, table), tName);
				e->symTypeRef = makeSymbol_Copy(e->symTypeRef);
				e->symTypeRef->bindingName = malloc(128);
				strcpy(e->symTypeRef->bindingName, getFullStr(e->val.binary.lhs, table));
				strcat(e->symTypeRef->bindingName, ".val[");
				strcat(e->symTypeRef->bindingName, tName);
				strcat(e->symTypeRef->bindingName, "]");
			}
...
void genStructureHelper(char *tName, char *tMods, SYMBOL *sym, int depth, char *typeName, bool isTypeDef, symTable *table)
{
		...
		if (strlen(tMods) != 0 && *tMods == '[')
		{
			...
			if(isTypeDef){
				printf("struct %s {\n", typeName);
				if(strcmp(table->bindingsList->bindingName, typeName) != 0)
				{
					copy->bindingName = typeName;
			...
			if(*(tMods+1) == ']')
			{
				...
				if(strlen(tMods) == 0 || *tMods != '[')
				{
					...
					genStructureHelper(tName, tMods, copy, depth+1, name1, true, table);
					prettyTabular(depth);
					printf("};\n");	
					prettyTabular(depth);
					printf("struct %s *val;\n", name1);
...
//determines if two struct types are equivalent: used to determine if a previously declared binding can be used
...
//looks for an existing compatible binding an returns its name
...
void codegenVarDecl(DECLARATION *decl, symTable *table, int depth)
{
	...
	if(parent != NULL)
	{
		...
		if(table->next->next != NULL && decl->val.right != NULL)
		{
			tName = getNewBinding();
			if(strlen(tMods) == 0 || *tMods != '[')
			{
				printf("struct %s %s = %s;\n", parent->bindingName, tName, getFullStr(decl->val.right, table));
				structAssignHelper(parent, NULL, tmp->bindingName, NULL, depth, table);
				structAssignHelper(parent, parent, tmp->bindingName, tName, depth, table);
			}
			else if(*(tMods+1) == ']')
			{
				printf("struct %s %s = %s;\n", parent->bindingName, tName, getFullStr(decl->val.right, table));
				sliceAssignHelper(tmp->bindingName, NULL, depth);
				sliceAssignHelper(tmp->bindingName, tName, depth);
			}
			else{
				printf("struct %s %s = %s;\n", parent->bindingName, tName, getFullStr(decl->val.right, table));
				arrayAssignHelper(parent, NULL, tMods, depth, tmp->bindingName, NULL, table);
				arrayAssignHelper(parent, parent, tMods, depth, tmp->bindingName, tName, table);
			}
		}
		else if(table->next->next != NULL)
		{
			if(strlen(tMods) == 0 || *tMods != '[')
			{
				structAssignHelper(parent, NULL, tmp->bindingName, NULL, depth, table);
			...
			struct declList *dtmp = malloc(sizeof(struct declList));
			dtmp->decl = decl;
			dtmp->tName = NULL;
			dtmp->tMods = tMods;
			dtmp->ref = tmp;
	...
	else if(tmp->kind == varstructSym)//"inline" struct definition, can be grouped in an array structure or the like
	{
		...
		if(strlen(tMods) == 0 || *tMods != '[')
		{
			...
			if(table->next->next != NULL) structAssignHelper(tmp, NULL, tmp->bindingName, NULL, depth, table);
		}
		else{
			prettyTabular(depth);
			...
			else if(strcmp(tName, "string") == 0)
			{
				...
				{ 
					...
					else
					{
						tName = getNewBinding();
						printf("struct %s %s = %s;\n", parent->bindingName, tName, getFullStr(decl->val.right, table));
						structAssignHelper(parent, decl->val.right->symTypeRef, tmp->bindingName, tName, depth, table);
					}
				}
			...
			if(table->next->next != NULL){
				if(decl->val.right == NULL){
					...
					if(*(tMods+1) == ']')
					{
						...
						printf("struct %s %s = %s;\n", parent->bindingName, tName, getFullStr(decl->val.right, table));
						arrayAssignHelper(tmp, NULL, tMods, depth, tmp->bindingName, NULL, table);
						arrayAssignHelper(tmp, tmp, tMods, depth, tmp->bindingName, tName, table);
					}
				}
			}
		}
...
void codegenTypeDecl(DECLARATION *decl, symTable *table, int depth)
{
	...
	if(strlen(findExistingBinding(tmp, table)) != 0)
		return;
	/*avoid the use of reserved keywords in C*/
	tmp->bindingName = strcat(getNewBinding(), tmp->name);
...
void codegenFuncDeclArgs(SYMBOL *args, symTable *table)
{
	...
	else
	{
		...
		if(strcmp(tName, "int") == 0)
			...
			{
				printf("char %s", args->bindingName);
			}
			else if(strcmp(tName, "string") == 0)
			{
				printf("char *%s", args->bindingName);
			...
			{
				printf("bool %s", args->bindingName);
			}
	}
...
void codegenFuncHeader(DECLARATION *decl, symTable *table, int depth)
{
	...
	if(strlen(tName) != 0)
	{
		if(tmp->val.func.returnSymRef->t->gType == arrayType || tmp->val.func.returnSymRef->t->gType == sliceType || tmp->val.func.returnSymRef->t->gType == structType || strcmp(getTypeName(tmp->val.func.returnSymRef), "struct") == 0)
		{
			printf("struct ");
		}
		printf( "%s %s(", tName, tmp->bindingName);
	}
	else if(tmp->val.func.returnSymRef->val.parentSym != NULL && tmp->val.func.returnSymRef->val.parentSym->kind == varstructSym)
	{
		SYMBOL *copy = makeSymbol_Copy(tmp->val.func.returnSymRef->val.parentSym);
			copy->bindingName = getNewBinding();
			copy->next = table->bindingsList;
			table->bindingsList = copy;
			table->bindingsSize++;
			printf("struct %s{\n", copy->bindingName);
			codegenStructHelper(tmp->val.func.returnSymRef->val.parentSym->val.structFields, depth+1, table);
			printf("};\n");
			printf( "struct %s %s(", copy->bindingName, tmp->bindingName);
	}
	...
	if(decl->val.f->body != NULL)
	{
		SYMBOL *tmp1, *tmp2;
		tmp1 = tmp->val.func.funcParams;
		while(tmp1 != NULL)
		{
...
void codegenFuncDecl(DECLARATION *decl, symTable *table, int depth)
{
	...
	{
		tName = findExistingBinding(tmp->val.func.returnSymRef->val.parentSym, table);
			printf( "struct %s %s(", tName, tmp->bindingName);
	}
	else if(tmp->val.func.returnSymRef->t->gType != nilType)
	{
		tName = getTypeName(tmp->val.func.returnSymRef);
	...
	if(strcmp("main", tmp->bindingName) == 0)
	{
		while(dinits != NULL)
		{
			if(strlen(dinits->tMods) == 0 && dinits->tName == NULL)
			{
				if(dinits->parent != NULL)
				{
					if(dinits->decl->val.right != NULL)
					{
						dinits->tName = getNewBinding();
						printf("struct %s %s = %s;\n", dinits->parent->bindingName, dinits->tName, getFullStr(dinits->decl->val.right, table));
						structAssignHelper(dinits->parent, NULL, dinits->ref->bindingName, NULL, depth, table);
						structAssignHelper(dinits->parent, dinits->parent, dinits->ref->bindingName, dinits->tName, depth, table);	
					}
					else{
						structAssignHelper(dinits->parent, NULL, dinits->ref->bindingName, NULL, depth, table);
					}
					...
					structAssignHelper(dinits->ref, NULL, dinits->ref->bindingName, NULL, depth, table);
				}
			}
			else if(strlen(dinits->tMods) != 0)
			{
				if(dinits->parent != NULL)
				{
					if(dinits->decl->val.right != NULL)
					{
						if(*(dinits->tMods+1) == ']')
						{
							dinits->tName = getNewBinding();
							printf("struct %s %s = %s;\n", dinits->parent->bindingName, dinits->tName, getFullStr(dinits->decl->val.right, table));
							sliceAssignHelper(dinits->ref->bindingName, NULL, depth);
							sliceAssignHelper(dinits->ref->bindingName, dinits->tName, depth);
						}
						else{
							dinits->tName = getNewBinding();
							printf("struct %s %s = %s;\n", dinits->parent->bindingName, dinits->tName, getFullStr(dinits->decl->val.right, table));
							arrayAssignHelper(dinits->parent, NULL, dinits->tMods, depth, dinits->ref->bindingName, NULL, table);
							arrayAssignHelper(dinits->parent, dinits->parent, dinits->tMods, depth, dinits->ref->bindingName, dinits->tName, table);
						}
					}
					else{
						if(*(dinits->tMods+1) == ']')
					...
					if(dinits->decl->val.right ==NULL){
						if(*(dinits->tMods+1) == ']')
						{
							...
							arrayAssignHelper(dinits->ref, NULL, dinits->tMods, depth, dinits->ref->bindingName, NULL, table);
						}
					}
					else{
						if(*(dinits->tMods+1) == ']')
						{
							...
							printf("struct %s %s = %s;\n", dinits->parent->bindingName, dinits->tName, getFullStr(dinits->decl->val.right, table));
							arrayAssignHelper(dinits->ref, NULL, dinits->tMods, depth, dinits->ref->bindingName, NULL, table);
							arrayAssignHelper(dinits->ref, dinits->ref, dinits->tMods, depth, dinits->ref->bindingName, dinits->tName, table);
						}
					}
				}
			}
			else{
				if(strcmp(dinits->tName, "float64") == 0)
				{
					if(dinits->decl->val.right == NULL)
					{
						printf("%s = 0.0;\n", dinits->ref->bindingName);
					}
					else{
						printf("%s = ", dinits->ref->bindingName);
						codegenExpression(dinits->decl->val.right, table);
						printf(";\n");
					}
				}
				else if(strcmp(dinits->tName, "rune") == 0)
				{
					if(dinits->decl->val.right == NULL)
					{
						printf("%s = 0;\n", dinits->ref->bindingName);
				...
				else if(strcmp(dinits->tName, "string") == 0)
				{
					if(dinits->decl->val.right == NULL)
					{
						printf("%s = malloc(128);\n", dinits->ref->bindingName);
						...
						printf("strcpy(%s, ", dinits->ref->bindingName);
						codegenExpression(dinits->decl->val.right, table);
						printf(");\n");
					}
				}
				else if(strcmp(dinits->tName, "bool") == 0 || strcmp(dinits->tName, "int") == 0)
				{
					if(dinits->decl->val.right == NULL)
					{
						printf("%s = 0;\n", dinits->ref->bindingName);
					...
					if(dinits->decl->val.right != NULL)
					{
						dinits->tName = getNewBinding();
						printf("struct %s %s = %s;\n", dinits->parent->bindingName, dinits->tName, getFullStr(dinits->decl->val.right, table));
						dinits->parent = getSymbol(table, dinits->tName, typeSym);
						structAssignHelper(dinits->parent, NULL, dinits->ref->bindingName, NULL, depth, table);
						structAssignHelper(dinits->parent, dinits->parent, dinits->ref->bindingName, dinits->tName, depth, table);
					}
					else{
						structAssignHelper(getSymbol(table, dinits->tName, typeSym), NULL, dinits->ref->bindingName, NULL, depth, table);
					}
				}
			}
			dinits = dinits->next;
...
void sliceAppendHelper(EXP *root, symTable *table, int depth)
{
	...
	elementCheck(target, table, depth);
	elementCheck(value, table, depth);
	...
	printf("}\n");
	prettyTabular(depth);
	printf("else{ %s.cap = %s.cap; %s.val = %s.val;}\n", newName, name, newName, name);
	...
	printf(";\n");
	root->symTypeRef = makeSymbol_Copy(root->symTypeRef);
    ...
    while(1)
    {
        ...
        {//move up hierarchy
            if(cursym->kind == funcSym)
            {
                ...
                if(cursym->val.parentSym->kind == structSym || cursym->val.parentSym->kind == varstructSym){
                    strcpy(typename, "struct");
                    return typename;
                }
                cursym = cursym->val.parentSym;
                cur = cursym->t;
...
void codegenAssign(STATEMENT *stmt, int depth){
	...
	if(parent == NULL)
	{
		...
		if(strlen(tMods) != 0 && *(tMods) == '[')
		{
			if(*(tMods+1) == ']')
			{
				printf("struct %s %s = %s;\n", parent->bindingName, tmpSym->bindingName, getFullStr(stmt->val.assignment.value, stmt->localScope));
			}
			else{
				printf("struct %s %s = %s;\n", parent->bindingName, tmpSym->bindingName, getFullStr(stmt->val.assignment.value, stmt->localScope));
			}
...
void codegenAssignLast(STATEMENT *stmt, int depth)
{
	...
	char *tMods = getTypeModifiers(stmt->val.assignment.identifier->symTypeRef);
	if(strcmp(tmp, "_") == 0)
	{
		return;
	}
	SYMBOL *parent = findExistingBindingSym(stmt->val.assignment.identifier->symTypeRef, stmt->localScope);
	
		if(parent != NULL && strlen(tMods) != 0 && *(tMods) == '[')
		{
			if(*(tMods+1) == ']')
			{
				sliceAssignHelper(tmp, stmt->val.assignment.value->symTypeRef->bindingName, depth);
			}
			else{
				arrayAssignHelper(parent, parent, tMods, depth, tmp, stmt->val.assignment.value->symTypeRef->bindingName, stmt->localScope);
			}
		}
		else if(parent != NULL && (parent->kind == structSym || parent->kind == varstructSym))
		{
			structAssignHelper(parent, parent, tmp, stmt->val.assignment.value->symTypeRef->bindingName, depth, stmt->localScope);
		}
...
void codegenQuickDecl(STATEMENT *stmt, int depth){
	...
	if(!(stmt->val.assignment.identifier->symTypeRef->isNew))
	{
		...
		stmt->val.assignment.identifier->symTypeRef->isNew = 0;
		stmt->val.assignment.identifier->symTypeRef->bindingName = strcat(getNewBinding(), stmt->val.assignment.identifier->val.identifier);
		// char *tName, *tMods;
		// SYMBOL *parent;
		...
		if(parent != NULL && strlen(tMods) != 0 && *(tMods) == '[')
		{
			...
			{
				printf("struct %s %s;\n", parent->bindingName, stmt->val.assignment.identifier->symTypeRef->bindingName);
				sliceAssignHelper(stmt->val.assignment.identifier->symTypeRef->bindingName, NULL, depth);
				sliceAssignHelper(stmt->val.assignment.identifier->symTypeRef->bindingName, stmt->val.assignment.value->symTypeRef->bindingName, depth);
			}
			else{
				printf("struct %s %s;\n", parent->bindingName, stmt->val.assignment.identifier->symTypeRef->bindingName);
				arrayAssignHelper(parent, NULL, tMods, depth, stmt->val.assignment.identifier->symTypeRef->bindingName, NULL, stmt->localScope);
				arrayAssignHelper(parent, parent, tMods, depth, stmt->val.assignment.identifier->symTypeRef->bindingName, stmt->val.assignment.value->symTypeRef->bindingName, stmt->localScope);
			}
		}
		else if(parent != NULL && (parent->kind == structSym || parent->kind == varstructSym))
		{
				printf("struct %s %s;\n", parent->bindingName, stmt->val.assignment.identifier->symTypeRef->bindingName);
				structAssignHelper(parent, NULL, stmt->val.assignment.identifier->symTypeRef->bindingName, NULL, depth, stmt->localScope);

			structAssignHelper(parent, parent, stmt->val.assignment.identifier->symTypeRef->bindingName, stmt->val.assignment.value->symTypeRef->bindingName, depth, stmt->localScope);
		}
		else{
			char *tName = getTypeName(stmt->val.assignment.identifier->symTypeRef);
			...
			{
				printf("%s %s = %s;\n",tName, stmt->val.assignment.identifier->symTypeRef->bindingName, stmt->val.assignment.value->symTypeRef->bindingName);
			}
		}
	}
...
char *rawString_Helper(char *s, int len)
{
...
char *stdString_Helper(char *s, int len)
{
	return s;
	char *tmp = malloc(len*2);
	tmp[0] = '"';
...
void topLevelType(DECLARATION *decl, symTable *table)
{
	if(decl == NULL) return;
	topLevelType(decl->chain, table);
	topLevelType(decl->next, table);
	if(decl->d != typeDecl) return;
				SYMBOL *tmp = getSymbol(table, decl->identifier, typeSym);
				...
				if(tmp->kind == structSym)
				{
					...
					if(strlen(tMods) == 0 || *tMods != '[')
					{
						prettyTabular(0);
						printf("struct %s {\n", tmp->bindingName);
						codegenStructHelper(tmp->val.structFields, 0+1, table);
						prettyTabular(0);
						printf("};\n");
					}
					else{
						prettyTabular(0);
						genStructureHelper("struct", tMods, tmp, 1, tmp->bindingName, true, table);
						prettyTabular(0);
						printf("};\n");
					}
...
void codegenExpression(EXP *e, symTable *table){
	...
	switch (e->kind){
		...
		case floatExp:
			...
			break;  
		case strExp: 
			printf("%s", stdString_Helper(e->val.strLiteral, strlen(e->val.strLiteral)));
			break; 
		case rawstrExp:
			e->val.strLiteral = rawString_Helper(e->val.strLiteral, strlen(e->val.strLiteral));
		...
		*/
		...
		case funcBlockExp:
			...
			if(parent == NULL)
			{
				...
				if(strcmp(tName, "float64") == 0)
				{
					...
					tName = getTypeName(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value->symTypeRef);
					if(strcmp(tName, "string") == 0)
					{
						printf("atof(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
						break;
					}
				}
				else if(strcmp(tName, "rune") == 0)
				...
				else if(strcmp(tName, "string") == 0)
				{
					...
					strcpy(parent->bindingName, "char *");
					tName = getTypeName(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value->symTypeRef);
					if(strcmp(tName, "int") == 0 || strcmp(tName, "bool") == 0)
					{
						printf("my_itoa(");
						codegenExpression(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table);
						printf(")");
					}
					else if(strcmp(tName, "float64") == 0)
					{
				...
				else if(strcmp(tName, "bool") == 0)
				{
					...
					if(strcmp(tName, "string") == 0)
					{
						printf("atoi(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
						break;
					}
				}
		...
		case appendExp:
			...
			break;
		case lenExp:
			tName = getTypeName(e->val.binary.rhs->symTypeRef);
			if(strcmp(tName, "string") == 0 && strlen(getTypeModifiers(e->val.binary.rhs->symTypeRef)) == 0)
			{
				printf("strlen(");
				codegenExpression(e->val.binary.rhs, table);
				printf(")");
		...
		case capExp:
			codegenExpression(e->val.binary.rhs, table);
			if(*(getTypeModifiers(e->val.binary.rhs->symTypeRef)+1) == ']')
				printf(".cap");
			else{
				printf( ".size");
			}
...
char *getFullStr(EXP *e, symTable *table)
{
	...
	switch(e->kind){
        ...
        case floatExp:
			...
			return returnVal;
        case strExp:
			sprintf(returnVal, "%s", stdString_Helper(e->val.strLiteral, strlen(e->val.strLiteral)));
			return returnVal;
        case rawstrExp:
			e->val.strLiteral = rawString_Helper(e->val.strLiteral, strlen(e->val.strLiteral));
		...
		case elementExp:
			sprintf(returnVal, "%s", e->symTypeRef->bindingName);
			return returnVal;
		case invocExp:
		...
		case funcBlockExp:
			...
			if(ref == NULL)
			{
				...
				if(strcmp(tName, "float64") == 0)
				{
					...
					if(strcmp(tName, "string") == 0)
					{
						sprintf(returnVal, "atof(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
						return returnVal;
					}
				}
				else if(strcmp(tName, "rune") == 0)
				...
				else if(strcmp(tName, "string") == 0)
				{
					...
					if(strcmp(tName, "int") == 0 || strcmp(tName, "bool") == 0)
					{
						sprintf(returnVal, "my_itoa(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
					}
					else if(strcmp(tName, "float64") == 0)
					{
						sprintf(returnVal, "ftoa(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
					}
					else if(strcmp(tName, "rune") == 0)
					{
						sprintf(returnVal, "ctoa(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
					}
					return returnVal;
				}
				else if(strcmp(tName, "bool") == 0)
				{
					...
					if(strcmp(tName, "string") == 0)
					{
						sprintf(returnVal, "atoi(%s)", getFullStr(e->val.fnblock.fn->params->val.fnCallBlock->val.expblock.value, table));
						return returnVal;
					}
				...
				if(e->val.fnblock.fn->params != NULL && e->val.fnblock.fn->params->val.fnCallBlock != NULL)
				{
					strcat(returnVal, strFuncCall(e->val.fnblock.fn->params->val.fnCallBlock, ref, table));
					strcat(returnVal, ")");
				}
			}
			else{
				...
				if(e->val.fnblock.fn->params != NULL && e->val.fnblock.fn->params->val.fnCallBlock != NULL)
				{
					strcat(returnVal, strFuncCall(e->val.fnblock.fn->params->val.fnCallBlock, ref->val.func.funcParams, table));
				}
				strcat(returnVal, ")");
			}
			return returnVal;
		case lenExp:
			...
			if(strcmp(tName, "string") == 0 && strlen(getTypeModifiers(e->val.binary.rhs->symTypeRef)) == 0)
			{
				sprintf(returnVal, "strlen(%s)", tmp1);
			}
			else{
		...
		case uxorExp:
			//As far as I can tell, 1111111... in C
			sprintf(returnVal, "-1^(%s)", getFullStr(e->val.binary.rhs, table));
			return returnVal;
		case funcExp:
		default:
