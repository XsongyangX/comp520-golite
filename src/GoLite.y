...
%type <decval> progdefs topdecl dec blockidents decdistributed typelist typedec typedistributed
%type <stmtval> stmts stmt ifstmt elsestmt switchstmt switchbody forstmt condstmt asnexps returnstmt simplestmt
...
access          : access '.' tIDENTIFIER {Exp id = makeEXP_id($3); $$ = makeEXP_invoc($1, id);}
                | access '.' tIDENTIFIER '(' exps ')' {Exp id = makeEXP_func($3, 0, $5); $$ = makeEXP_invoc($1, id);}
                | tIDENTIFIER '(' exps ')' {$$ = makeEXP_func($1, 0, $3);}
                | tIDENTIFIER {$$ = makeEXP_id($1);}
                | access '.' tIDENTIFIER '[' exp ']' %prec UNARY {Exp id = makeEXP_id($3); $$ = makeEXP_index($5); $$ = makeEXP_element(id, $$); $$ = makeEXP_invoc($1, $$);}
                | access '.' tIDENTIFIER '(' exps ')' '[' exp ']' %prec UNARY {Exp id = makeEXP_func($3, 0, $5); $$ = makeEXP_index($8); $$ = makeEXP_element(id, $$); $$ = makeEXP_invoc($1, id);}
                | tIDENTIFIER '(' exps ')' '[' exp ']' %prec UNARY {$$ = makeEXP_func($1, 0, $3); Exp *tmp = makeEXP_index($6); $$ = makeEXP_element($$, tmp);}
                | tIDENTIFIER '[' exp ']' %prec UNARY {$$ = makeEXP_index($3); Exp *tmp = makeEXP_id($1); $$ = makeEXP_element(tmp, $$);}
                | access '.' tIDENTIFIER '[' exp ':' exp ']' %prec UNARY {Exp id = makeEXP_id($3); $$ = makeEXP_range($5, $7); $$ = makeEXP_element(id, $$); $$ = makeEXP_invoc($1, $$);}
                | access '.' tIDENTIFIER '(' exps ')' '[' exp ':' exp ']' %prec UNARY {Exp id = makeEXP_func($3, 0, $5); $$ = makeEXP_range($8,$10); $$ = makeEXP_element(id, $$); $$ = makeEXP_invoc($1, id);}
                | tIDENTIFIER '(' exps ')' '[' exp ':' exp ']' %prec UNARY {$$ = makeEXP_func($1, 0, $3); Exp *tmp = makeEXP_range($6, $8); $$ = makeEXP_element($$, tmp);}
                | tIDENTIFIER '[' exp ':' exp ']' %prec UNARY {$$ = makeEXP_range($3, $5); Exp *tmp = makeEXP_id($1); $$ = makeEXP_element(tmp, $$);}
...
simplestmt      : exp ';' {$$ = makeSTMT_exp(yylineno, $1);} 
                | tIDENTIFIER tDECREMENT ';' {Exp ident = makeEXP_id($1); Exp one = makeEXP_int(1); 
                    Exp identMinus = makeEXP_sub(ident, one);  $$ = makeSTMT_assmt(yylineno, $1, identMinus);}
                | tIDENTIFIER tINCREMENT ';' {Exp ident = makeEXP_id($1); Exp one = makeEXP_int(1); 
                    Exp identPlus = makeEXP_plus(ident, one);  $$ = makeSTMT_assmt(yylineno, $1, identPlus);}
                | asnexps ';' {$$ = $1;}
                ;
...
elsestmt        : tELSE tIF exp '{' stmts '}' elsestmt {$$ = makeSTMT_elif(yylineno, $3, NULL, $5, $7);}
                ...
                | tELSE exp '{' stmts '}' ';' {$$ = makeSTMT_else(yylineno, $2);}
                | ';' {$$ = NULL;}
                ;
...
returnstmt      : tRETURN ';' {$$ = makeSTMT_return(yylineno);}
...
asnexps         : wideridents '=' exps {$$ = makeSTMT_blockassign(yylineno, $1, $3);}
                | idents tCOLONASSIGN exps {Decl *d = makeDECL_blocknotype(yylineno, $1, $3); $$ = makeSTMT_decl(yylineno, d);}
                | tIDENTIFIER tPLUSASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_plus(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tMINUSASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_sub(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tTIMESASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_times(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tDIVASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_div(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tMODASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_mod(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tANDASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_band(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tORASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_bor(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tPOWASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_xor(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tAMPPOWASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_andnot(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tLSHIFTASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_lshift(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                | tIDENTIFIER tRSHIFTASSIGN exp {Exp *id = makeEXP_id($1); 
                    newExp = makeEXP_rshift(id, $3);  $$ = makeSTMT_assmt(yylineno, id, newExp);}
                ;

condstmt        : idents tCOLONASSIGN exps ';' {Decl *d = makeDECL_blocknotype(yylineno, $1, $3); $$ = makeSTMT_decl(yylineno, d);}
                | tIDENTIFIER '(' idents ')' ';' {Exp *e = makeEXP_func($1, 0, $3); $$ = makeSTMT_exp(yylineno, e);}
